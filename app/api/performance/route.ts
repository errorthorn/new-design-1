...
// username = email) for the time-spent total, and (AI Feedback plan §6
// Phase E.2) the student's own recent speaking_feedback rows for the
// "Recent Speaking Club Feedback" card.
//
...
//    qualifying activity of any kind (for Speaking Club: an attended
//    session — see lib/speaking-club-attendance.ts), across all-time data — deliberately
...
import { getAttendedUtcDayKeys } from "@/lib/speaking-club-attendance";
...
export async function GET(req: NextRequest) {
  ...
  async function fetchStreakDates() {
    ...
    try {
      const [{ data: quizDates }, speakingDayKeys] = await Promise.all([
        supabaseServer.from("quiz_attempts").select("completed_at").eq("user_email", user.email).not("completed_at", "is", null),
        // Speaking Club days count only for a real ATTENDED session (5+ min
        // with a partner — lib/speaking-club-attendance.ts), not for every
        // call report: joining an empty room or dropping out after seconds
        // used to keep the streak alive. Its own catch so a hiccup here
        // costs only the Speaking Club days, not quiz/mock/vocab too.
        getAttendedUtcDayKeys(user.email).catch(() => [] as string[]),
      ]);
      for (const r of quizDates ?? []) if (r.completed_at) dates.add(dateKey(r.completed_at));
      for (const key of speakingDayKeys) dates.add(key);
  ...
  // AI Feedback plan §6 Phase E.2: "extend /api/performance itself to
  // include this list in its response ... rather than the frontend
  // making a separate fetch" — this page already aggregates multiple
  // sources server-side, so this fits its existing job. Explicit LIMIT
  // 10 per the plan (don't fetch a student's entire history on every
  // page load) — a "see full history" view would be a separate paginated
  // page, not this summary card.
  async function fetchSpeakingClubFeedback() {
    try {
      const { data, error } = await supabaseServer
        .from("speaking_feedback")
        .select("id, created_at, status, feedback_summary, mistake_count")
        .eq("student_username", user.email)
  ...
  const [quiz, mock, vocabBattle, speakingSeconds, activeDates, trend, speakingClubFeedback] = light
    ? await Promise.all([
        ...
        // Home's cards (see the ?light=1 comment above) never render this
        // list either — same "unused work" reasoning applies here.
        Promise.resolve([] as Awaited<ReturnType<typeof fetchSpeakingClubFeedback>>),
      ])
    : await Promise.all([
        fetchQuiz(),
        fetchMock(),
