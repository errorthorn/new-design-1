...
export async function POST(req: NextRequest) {
  ...
  // Compared only against rounds of the same length: rounds went from 10 to
  // 20 questions, and a 20-question score is naturally ~2x a 10-question
  // one, so without this every new round would "beat" the old 10-word best.
  const priorBestRes = await db.execute({
    sql: `SELECT MAX(score) AS best FROM vocab_battle_attempts WHERE user_id = ? AND mode = 'solo' AND total_words = ?`,
    args: [user.id, totalWords],
  });
