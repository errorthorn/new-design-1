...
export async function GET() {
  ...
  // Streak / attended-session stats for the Home card. Best-effort: a
  // failure here must never take the shift list down with it.
  const attendance = await getAttendanceStats(user.email).catch(() => null);

  return NextResponse.json({ shifts: shiftSummaries, activeShift: active, attendance });
}
