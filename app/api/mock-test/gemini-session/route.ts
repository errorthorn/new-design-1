...
import { checkSlotWindow, markBookingInProgress, isAtLiveCapacity } from "@/lib/mock-test-slots";
...
export async function POST(req: NextRequest) {
  ...
  // Free-tier concurrency control: the student must be inside their booked
  // slot window (see lib/mock-test-slots.ts) — this is the one place that
  // actually enforces it; the client only mirrors it for UX. needsNewSlot
  // tells the client whether to silently re-fetch a slot (their old one
  // expired or never existed) versus just showing "come back at <time>".
  const slotCheck = await checkSlotWindow(studentId);
  if (!slotCheck.ok) {
    return NextResponse.json(
      {
        error: slotCheck.message,
        needsNewSlot: slotCheck.reason === "expired" || slotCheck.reason === "no_booking",
        slotStart: slotCheck.reason === "too_early" ? slotCheck.slotStart : undefined,
      },
      { status: 403 }
    );
  }

  // Second, independent safety net: even if the slot schedule above somehow
  // let too many people's windows overlap, never spend a Gemini token past
  // the actual configured concurrency limit.
  if (await isAtLiveCapacity()) {
    return NextResponse.json(
      {
        error: "The test system is at full capacity for a moment. Please wait about a minute and try again.",
        quotaExceeded: true,
      },
      { status: 429 }
    );
  ...
  // Now that the attempt has actually been created, mark the slot booking
  // in_progress too — after the insert (not before) so a failed insert
  // above never leaves a booking flipped to in_progress with no matching
  // attempt behind it.
  await markBookingInProgress(slotCheck.bookingId);
  ...
  const advancePartTool = {
    functionDeclarations: [
      ...
      },
      {
        name: "end_test",
        description:
          "Call this exactly once, right as you deliver your final closing remarks after the last Part 3 question (thanking the student and telling them the test is complete). Do not call it any earlier, and do not call it more than once.",
        parameters: {
          type: Type.OBJECT,
          properties: {},
        },
  ...
  const systemInstruction = hasAnyQuestions
    ? [
        ...
        "- You MUST call the end_test function right as you deliver your final closing remarks at the very end of Part 3. Never call it earlier, and never call it more than once.",
        "",
