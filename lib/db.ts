    ...
    ready = client
      ...
      .then(() =>
        Promise.all([
          ...
          // Marks a sample/test account (e.g. one created to try out a
          // feature before rolling it out to real students) — toggled on
          // /admin/members. Excluded from listSubscribedUsers() so Speaking
          // Club auto-pair (see /api/admin/speaking-club/auto-pair) never
          // proposes pairing a real student with a test account, even while
          // that test account's own subscription is switched on to try
          // something out. Doesn't affect the account otherwise — it can
          // still sign in, hold a subscription, and be assigned a shift by
          // hand for deliberate testing.
          addColumnIfMissing("users", "is_test INTEGER NOT NULL DEFAULT 0"),
        ])
      )
      .then(() =>
      ...
      .then(() =>
        ...
        // /admin/vocab-words). daily_date (YYYY-MM-DD) + topic together
        // describe which day's vocabulary set a word belongs to — e.g.
        // every word tagged daily_date='2026-09-18', topic='Travel' is
        // that day's topic-based batch, shown together in the "Previous
        // Vocabulary" browser on /dashboard/vocab. Multiple words share
        // the same daily_date (a whole day's set, not a single "Word of
        // the Day" — see the migration below for why the old unique
        // index on daily_date was removed). A word with no daily_date is
        // just a general-pool word, not tied to any specific day's set.
        // Synonyms/examples are capped at two each per the practice-card
        // design, so plain columns (not a child table) keep reads/writes
        // simple.
        client.execute(`
      ...
      .then(() =>
        // Previous Vocabulary feature: a day's vocab set is now MULTIPLE
        // words sharing one daily_date (a topic-based batch), not a
        // single featured word — so the old UNIQUE constraint below,
        // which allowed at most one word per date, has to go first
        // (SQLite has no "ALTER INDEX", so this is drop-then-recreate).
        // Safe against a database that never had this index (a fresh
        // install skips straight to CREATE TABLE above, which never
        // created it either) — DROP INDEX IF EXISTS is a no-op there.
        client.execute(`DROP INDEX IF EXISTS idx_vocab_words_daily_date;`)
      )
      .then(() =>
        // Non-unique now — see the DROP above. Still indexed, since
        // /api/vocab's "group into daily sets for Previous Vocabulary"
        // query and /api/vocab/[id]/progress's lookups both filter/sort
        // on this column.
        client.execute(
          `CREATE INDEX IF NOT EXISTS idx_vocab_words_daily_date ON vocab_words (daily_date) WHERE daily_date IS NOT NULL;`
        )
      )
      .then(() =>
        // Previous Vocabulary feature: which topic a day's vocab set was
        // about (e.g. "Travel", "Business Meetings") — shown as the
        // heading for that day's group on /dashboard/vocab. Free text,
        // not a foreign key to a separate topics table: topics here are
        // just a label repeated across every word in the same day's
        // batch, not a reusable taxonomy admins pick from elsewhere.
        addColumnIfMissing("vocab_words", "topic TEXT")
      )
      .then(() =>
