...
// --- local speech detection (see onStudentSpeaking + onaudioprocess below) ---
// Root-mean-square volume, on a 0..1 scale of the float32 mic samples, above
// which a chunk counts as "loud enough to be speech" rather than background
// hiss/room noise. getUserMedia's noiseSuppression/echoCancellation already
// clean the signal up before this runs, so this can stay a fairly low bar.
const SPEECH_RMS_THRESHOLD = 0.015;
// Require this many consecutive loud 4096-sample chunks (~0.26s each at
// 16kHz, so ~0.5s total) before trusting it as real speech and not a single
// stray click/bump — short enough to still feel instant next to the 8s
// silence timeout, long enough to filter one-off noise spikes.
const LOUD_FRAMES_TO_CONFIRM = 2;
...
export class GeminiLiveSession {
  ...
  // Fires when the examiner calls the end_test tool (see
  // gemini-session/route.ts) — i.e. right as it delivers its closing
  // remarks at the end of Part 3. Lets the session page finish the test
  // automatically (same as the student tapping "Finish Test") instead of
  // requiring a manual click after the examiner says the test is over.
  onTestEnd?: () => void;
  ...
  // Fires as soon as Gemini's own speech-to-text produces any text for the
  // student's mic input — i.e. the student has started actually answering.
  // The session page uses this to cancel its silence-timeout watch (see
  // startSilenceWatch there), since Gemini's automatic turn-detection alone
  // can't be trusted to always come back on its own if the student never
  // makes a sound at all.
  // Historically this only fired from Gemini's own inputTranscription event
  // arriving over the WebSocket (see handleMessage below) — but that's a
  // round trip through Gemini's server-side turn detection + STT, and for
  // longer or slightly hesitant answers it can easily take longer than the
  // session page's 8s silence timeout to arrive, even though the student is
  // actively talking the whole time. That made the silence-repeat/move-on
  // cycle fire on top of a real answer instead of only on real silence. It
  // now ALSO fires instantly from local mic-volume detection in
  // onaudioprocess below, which doesn't depend on any server round trip —
  // whichever signal arrives first wins, and calling this twice for the
  // same utterance is harmless (see its usage in the session page).
  onStudentSpeaking?: () => void;
  ...
  // is gated. Set from outside via setMicSuppressed().
  private micSuppressed = false;
  // True while the examiner's own turn is actively producing audio (i.e.
  // it's mid-question or mid-instruction). Set automatically the moment
  // any audio arrives in a modelTurn and cleared on that turn's
  // turnComplete — see handleMessage. While true the mic is gated exactly
  // like micSuppressed above, so the student's mic audio never reaches
  // Gemini while the examiner is talking: previously a sound from the
  // student mid-question could trigger Gemini's own barge-in detection
  // (serverContent.interrupted) and cut the examiner off; now the
  // examiner always finishes its question, and the student's turn to
  // speak only really begins once the examiner's audio stops (matching
  // onExaminerTurnComplete, which is what actually starts the
  // silence-timeout watch on the session page).
  private examinerSpeaking = false;
  // Consecutive loud chunks seen so far in the current onaudioprocess run —
  // see SPEECH_RMS_THRESHOLD / LOUD_FRAMES_TO_CONFIRM above.
  ...
  private async startMic() {
    ...
    this.processor.onaudioprocess = (e) => {
      ...
      if (this.micSuppressed || this.examinerSpeaking) return;

      const input = e.inputBuffer.getChannelData(0);

      // Instant local speech detection — see onStudentSpeaking's comment
      // for why this doesn't wait on Gemini's inputTranscription instead.
      let sumSquares = 0;
      for (let i = 0; i < input.length; i++) sumSquares += input[i] * input[i];
      const rms = Math.sqrt(sumSquares / input.length);
      if (rms > SPEECH_RMS_THRESHOLD) {
        this.consecutiveLoudFrames++;
        if (this.consecutiveLoudFrames >= LOUD_FRAMES_TO_CONFIRM) {
          this.onStudentSpeaking?.();
        }
      } else {
  ...
  private async handleMessage(evt: MessageEvent, sourceWs: WebSocket) {
    ...
    // The examiner calling advance_part (Part 1→2 or 2→3) or end_test
    // (closing remarks) — see the tool declarations in
    // gemini-session/route.ts. Gemini expects a toolResponse acknowledging
    // every functionCall before it will continue the conversation.
    if (msg.toolCall) {
    ...
    if (sc.interrupted) {
      // With examinerSpeaking gating the mic (above), the student's audio
      // never reaches Gemini while the examiner is talking, so this
      // shouldn't fire from the student anymore. Kept as a safety net
      // (e.g. a genuine server-side hiccup) — still stop local playback
      // so nothing gets stuck, but don't treat it as meaningful.
      this.stopPlayback();
    }
    ...
    if (sc.modelTurn?.parts) {
      // The first audio of a new examiner turn — start gating the mic
      // immediately (see examinerSpeaking above) so nothing the student
      // says can interrupt the question that's about to play.
  ...
  // The examiner calling advance_part or end_test (see the tool
  // declarations in gemini-session/route.ts). Gemini expects a
  // toolResponse acknowledging every functionCall before it will continue
  // the conversation, regardless of whether we recognized the call.
  ...
  // How much longer the examiner's already-scheduled audio still has left
  // to play, in ms. Used by the session page to wait out the closing
  // remarks (see onTestEnd) before actually tearing the session down —
  // close()/stopPlayback() below stop audio immediately, which would
  // otherwise cut the examiner off mid-sentence.
  getRemainingPlaybackMs(): number {
    if (!this.playbackContext) return 0;
    return Math.max(0, (this.playbackQueueTime - this.playbackContext.currentTime) * 1000);
  }
