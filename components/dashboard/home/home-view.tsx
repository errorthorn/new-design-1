...
type SpeakingClubStatus = {
  ...
  // Attended-session stats (lib/speaking-club-attendance.ts); null if the
  // server couldn't compute them — the card simply omits that line.
  attendance?: {
...
// `user` comes from CurrentUserProvider (DashboardShell already fetched
// /api/auth/me for the sidebar) — this hook no longer fetches it a second
// time, it only owns the four requests genuinely specific to this page.
function useHomeData(user: MeUser | null | undefined) {
  const [data, setData] = useState<Omit<HomeData, "user">>({
    mockTest: undefined,
  ...
  useEffect(() => {
    ...
    // on the slowest of the four before anything shows up (that used to
    ...
    // ?countOnly=1 skips fetching every mistake's full question/options/
    // explanation text — this card only ever shows a number — see the
    // matching note in app/api/quiz/mistakes/route.ts.
    safeJson("/api/quiz/mistakes?countOnly=1").then((mistakes) => {
...
// ---------------------------------------------------------------------------
// Section label — same uppercase/tracking treatment as the sidebar's group
// headers (see DASHBOARD_NAV group.label in dashboard-shell.tsx), reused
// here so the home page's new groupings read as one system, not a new one.
// ---------------------------------------------------------------------------
function SectionLabel({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <p
      className={cn(
        "font-body text-[11px] font-semibold uppercase tracking-wider text-ink-soft/60 dark:text-cream/40",
        className
      )}
    >
      {children}
...
export function HomeView() {
  const user = useCurrentUser() as MeUser | null | undefined;
  ...
  return (
    <div>
      ...
      {/* Your progress — the at-a-glance stat cards */}
      <SectionLabel className="mt-8">Your progress</SectionLabel>
      ...
      {/* Mock Test & Speaking Club — this week's graded test plus today's
          speaking session, grouped together since both are the weekly
          subscription-driven activities */}
      <SectionLabel className="mt-8">Mock test &amp; speaking club</SectionLabel>
      ...
      {/* Study plan — its own section, kept separate from Mock Test/Speaking
          Club since it's self-directed rather than scheduled */}
      <SectionLabel className="mt-8">Study plan</SectionLabel>
      <div className="mt-3">
        <StudyPlanCard />
      </div>

      {/* Start practicing — quick shortcuts into daily practice */}
      <SectionLabel className="mt-8">Start practicing</SectionLabel>
      <div className="mt-3">
        <QuickLaunchCard />
      </div>
    </div>
...
function SpeakingClubCard({ loading, data }: { loading: boolean; data: HomeData }) {
  ...
  const attendance = data?.speakingClub?.attendance ?? null;
  ...
  return (
    <Card className="flex h-full flex-col">
      ...
      {/* Speaking Club attendance — the streak these sessions feed into is
          the "Activity streak" card above. Only shown once there's
          something to show, so a brand-new member doesn't see "0 · 0". */}
      {attendance && attendance.totalSessions > 0 && (
        <p className="mt-4 flex items-center gap-1.5 border-t border-ink/10 pt-3 font-body text-xs text-ink-soft dark:border-cream/10 dark:text-cream/60">
          <Flame size={13} className="text-leaf-700 dark:text-leaf-500" />
          {attendance.currentStreak > 1 ? `${attendance.currentStreak}-day streak · ` : ""}
          {attendance.last7Days} session{attendance.last7Days === 1 ? "" : "s"} in the last 7 days
          {attendance.attendedToday ? " · today ✓" : ""}
        </p>
      )}
    </Card>
  );
...
function MistakeCard({ loading, data }: { loading: boolean; data: HomeData }) {
  ...
  const hasData = data?.mistakes != null;
  const count = data?.mistakes?.count ?? 0;
